    ...
    <form wire:submit=save>
        ...
        @if($user_perfil !== "usuario")
            <div class="form-group">
                ...
                <button onsubmit=false type=button class='btn btn-sm btn-secondary' wire:click="buscarUsuarioPorDNI">Buscar usuario</button>
            
            @if(null !== $usuario)
                <p>Nombre: {{$usuario->persona->Nombre . " " . $usuario->persona->Apellido}}<p>
                <p>ID usuario: {{$usuario->persona->usuario->id_usuario}} </p>
            @endif
            </div>
        @endif
        <div class="form-group">
